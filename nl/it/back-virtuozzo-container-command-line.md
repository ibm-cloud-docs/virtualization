...
# Backup di un contenitore Virtuozzo nella riga di comando
{: #backing-up-a-virtuozzo-container-in-the-command-line}

Utilizza la seguente procedura per eseguire il backup di un contenitore su un server Virtuozzo.
...
2. Avvia il backup dopo che hai verificato la connettività al nodo hardware di backup.
...
Due cose da notare in merito al comando precedente:
1. Devi eseguire l'accesso ed eseguire il comando di backup sul server “destination” (il server sul quale desideri che stia il backup). Dopo che hai eseguito il comando, viene effettuata una richiesta per l'ID contenitore dal nodo hardware originale.
2. Viene utilizzata l'opzione `-e` per designare un singolo contenitore. Senza questa opzione, il comando esegue il backup di tutti i contenitori sul server host nella nuova ubicazione.
...
Puoi visualizzare i backup effettuati e archiviati sul server utilizzando il seguente comando:
